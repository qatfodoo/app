 ...
 * @param {Function} fn Callback function
 * @param {Citizen} citizen The voe citizen
 */
exports.unvote = function (id, fn, citizen) {
  Comment.findById(id).populate('author').exec(function (err, comment) {
    ...
    comment.unvote(citizen, valoration, function (err) {
      if (err) {
        log('Found error %s', err);
        return fn(err);
      }
 ...
 * @api public
 * @param {String} valoration Valoration of the comment.
 * @param {String} logMessage String for log message
 */
//FIXME: param quantity
function voteComment(id, fn, citizen, valoration, logMessage) {
  Comment.findById(id).populate('author').exec(function (err, comment) {
    ...
    log('%s comment %s', logMessage,comment.id);
    comment.vote(citizen, valoration, function (err) {
      if (err) return log('Found error %s', err), fn(err);
...
exports.upvote = function upvote(id, citizen, fn) {
  voteComment(id, fn, citizen,'positive','Upvoting');
};
...
exports.downvote = function downvote(id, citizen, fn) {
  voteComment(id, fn, citizen,'negative','Downvote');
};
