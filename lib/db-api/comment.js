...
var sendReplyEmail = require('lib/email-notification').sendReplyEmail;
...
exports.reply = function reply(commentId, reply, meta, fn) {
  ...
  .exec(function(err, comment) {
    ...
    comment.save(function(err, saved) {
      ...
      comment
      .populate('replies.author', 'id firstName lastName fullName email', function(err, comment) {
        if (err) {
          log('Found error %j', err);
          return fn(err);
        };
        ...
        sendReplyEmail(comment, reply, meta, function (err, reply) {
          if (err) {
            log('Found error %j', err);
            return fn(err);
          };
